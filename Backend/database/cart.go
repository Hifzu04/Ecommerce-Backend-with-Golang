...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	var getusercart []bson.M //in key val format
	...
	// pushing all items from the user's cart into the order_list of every order for that user.
...
func InstantBuyer(ctx context.Context, prodCollection, userCollection *mongo.Collection, productID primitive.ObjectID, UserID string) error {
	...
	err = prodCollection.FindOne(ctx, bson.D{{Key: "_id", Value: productID}}).Decode(&productDetail)
	if err != nil {
		log.Println(err)
	}
	...
	filter := bson.D{{Key: "_id", Value: id}}
	update := bson.D{{Key: "$push", Value: bson.D{{Key: "orders", Value: orderdetail}}}}
	_, err = userCollection.UpdateOne(ctx, filter, update)
	...
	filter2 := bson.D{{Key: "_id", Value: id}}
	update2 := bson.M{"$Push": bson.M{"orders.$[].order_list": productDetail}}
	_, err = userCollection.UpdateOne(ctx, filter2, update2)
	if err != nil {
		log.Println(err)
